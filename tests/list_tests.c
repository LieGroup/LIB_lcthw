...
char *test_push_pop()
{
	...
	mu_assert(List_count(list) == 3, "Wrong countafter push");
	...
	mu_assert(List_count(list) == 0, "Wrong countafter pop");
...
char *test_remove()
{
	// 只需要测试在List中间删除节点，因为首尾节点的删除在pop/unshift已经测试
	// TODO: 删除一个节点后，要验证新的前后节点对应关系，而不是首尾节点
